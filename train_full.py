...
from datasets import NIHCXR, MIMIC, NLMCXR, ours
from losses import CELoss, CELossTotal, CELossTotalEval, CELossTransfer, CELossShift
from models_ours import CNN, MVCNN, TNN, Classifier, Generator, ClsGen, ClsGenInt
...
def infer(data_loader, model, device='cpu', threshold=None):
    ...
    with torch.no_grad():
        ...
        for i, (source, target) in enumerate(prog_bar):
            ...
            if threshold != None:
                output = model(image=source[0], history=source[3],retrieval=source[4],threshold=threshold)
...
MODELS = 'Retrieval'  # Retrieval/CMN/CG/CR
...
if __name__ == "__main__":
    ...
    if MODELS in ['CR','Retrieval']:
        SOURCES = ['image', 'caption', 'label', 'history', 'retrieval']
        TARGETS = ['caption', 'label']
        KW_SRC = ['image', 'caption', 'label', 'history', 'retrieval']
        KW_TGT = None
        KW_OUT = None

    elif MODEL_NAME in ['ClsGen', 'ClsGenInt']:
    ...
    if DATASET_NAME == 'MIMIC':
        ...
        dataset = MIMIC('/workspace/data/files/', INPUT_SIZE, view_pos=['AP', 'PA', 'LATERAL'], max_views=MAX_VIEWS,
        ...
        dataset = NLMCXR('./Datasets/NLMCXR/', INPUT_SIZE, view_pos=['AP', 'PA', 'LATERAL'], max_views=MAX_VIEWS,
                         sources=SOURCES, targets=TARGETS)
        train_data, val_data, test_data = dataset.get_subsets(seed=123)

        VOCAB_SIZE = len(dataset.vocab)
        POSIT_SIZE = dataset.max_len
        COMMENT = 'MaxView{}_NumLabel{}_{}History'.format(MAX_VIEWS, NUM_LABELS, MODELS,
                                                          'No' if 'history' not in SOURCES else '')
        ...
        dataset = ours('./Datasets/ours/', INPUT_SIZE, view_pos=['AP', 'PA', 'LATERAL'], max_views=MAX_VIEWS,
                       sources=SOURCES, targets=TARGETS)
        train_data, val_data, test_data = dataset.get_subsets(seed=123)
    ...
    if MODEL_NAME == 'ClsGen':
        ...
        # model = ClsGen(cls_model, gen_model, NUM_LABELS, NUM_EMBEDS, NUM_HEADS)
        ...
        # clsgen_model = ClsGen(cls_model, gen_model, NUM_LABELS, NUM_EMBEDS, NUM_HEADS)
        ...
        # for param_tensor in int_model.state_dict():
        #     # 打印 key value字典
        #     print(param_tensor, '\t', int_model.state_dict()[param_tensor].size())

        if not RELOAD:
            checkpoint_path_from = 'checkpoints/{}_Transformer_MaxView2_NumLabel{}_{}.pt'.format(DATASET_NAME,
                                                                                                 NUM_LABELS, MODELS)
            # checkpoint_path_from = 'checkpoints/{}_Transformer_MaxView2_NumLabel{}.pt'.format(DATASET_NAME,
            #                                                                                      NUM_LABELS)

            # test_model = torch.load(checkpoint_path_from)
            # state_dict=test_model['model_state_dict']
            # # print((test_model.keys()))
            # # print((state_dict))
            #
            # # state_dict=test_model.model_state_dict()
            # # # print model's state_dict
            # print('testModel.state_dict:')
        ...
        model = Transformer(image_encoder=cnn, num_tokens=VOCAB_SIZE, num_posits=POSIT_SIZE,
                            ...
                            dropout=DROPOUT, num_layers_enc=NUM_LAYERS_ENC, num_layers_dec=NUM_LAYERS_DEC,
                            freeze_encoder=True)
        ...
        print('Reload From: {} | Last Epoch: {} | Validation Metric: {} | Test Metric: {}'.format(pretrained_from,
                                                                                                  last_epoch,
                                                                                                  best_metric,
                                                                                                  test_metric))
        ...
        diff_chexpert = LSTM_Attn(num_tokens=VOCAB_SIZE, embed_dim=NUM_EMBEDS, hidden_size=HIDDEN_SIZE,
                                  num_topics=NUM_LABELS, num_states=NUM_CLASSES, dropout=DROPOUT)
        ...
        model = ST(cnn, num_tokens=VOCAB_SIZE, fc_features=FC_FEATURES, embed_dim=NUM_EMBEDS, hidden_size=HIDDEN_SIZE,
    ...
    val_loader = data.DataLoader(val_data, batch_size=BATCH_SIZE, shuffle=False, num_workers=0)
    ...
    # checkpoint_path_from = 'checkpoints/{}_{}_{}_{}.pt'.format(DATASET_NAME,clsgenint,BACKBONE_NAME,COMMENT)
