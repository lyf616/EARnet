...
class MIMIC(data.Dataset): # MIMIC-CXR Dataset
    ...
    def __getitem__(self, idx):
        ...
        retrievals = self.retrieval[self.img_files[idx][0]]
        ...
        retrieval_info = []
        for retrieved_caption in retrievals:
            retrieval = retrieved_caption
            encoded_retrieval = [self.vocab.bos_id()] + self.vocab.encode(retrieval) + [self.vocab.eos_id()]
            retrieval = np.ones(self.max_len, dtype=np.int64) * self.vocab.pad_id()
            retrieval[:min(len(encoded_retrieval), self.max_len)] = encoded_retrieval[
                                                                    :min(len(encoded_retrieval), self.max_len)]
            retrieval_info.append(retrieval)
        ...
        for i in range(len(self.sources)):
            ...
            if self.sources[i] == 'retrieval':
                sources.append(retrieval_info)
    ...
    def __input_retrieval(self):
        with open(self.dir + 'retrieved_captions.json', encoding="utf-8") as f:
    ...
    def __input_data(self, binary_mode=True):
        ...
        self.__input_retrieval()

        
    def __generate_splits(self, test_size=0.2, seed=0, file_name='mimic-cxr-2.0.0-split.csv'):
        ...
        pid_sid = {}
        for i in range(len(data)):
            dicom_id = data[i,0].item()
            study_id = data[i,1].item()
            subject_id = data[i,2].item()
            split = data[i,3].item()
            ...
            if pid in pid_sid:
                pid_sid[pid].append((sid, dicom_id, split))
            else:
                pid_sid[pid] = [(sid, dicom_id, split)]

        # Write to train_val_list.txt and test_list.txt based on the official split
        for pid, studies in pid_sid.items():
            for study_id, dicom_id, split in studies:
                entry = f'p{pid}/s{study_id}\n'
                if split == 'train':
                    if ('p' + pid, 's' + study_id) in self.img_captions:
                        train_val_file.write(entry)
                elif split == 'test':
                    if ('p' + pid, 's' + study_id) in self.img_captions:
        ...
        # test_file = open(self.dir + 'test_list.txt', 'w')
        #
        # txt_file = self.dir + 'mimic-cxr-2.0.0-split.csv'
        # data = pd.read_csv(txt_file, dtype=object)
        # data = data.to_numpy().astype(str)
        #
...
class NLMCXR(data.Dataset):  # Open-I Dataset
    ...
    def __getitem__(self, idx):
        ...
        retrievals = self.retrieval[self.file_report[file_name]['image'][0] + '.png']
        features = self.feature[self.file_report[file_name]['image'][0] + '.png']
        ...
        encoded_target_info = [self.vocab.bos_id()] + self.vocab.encode(target_info) + [self.vocab.eos_id()]
        target_info = np.ones(self.max_len, dtype=np.int64) * self.vocab.pad_id()
        target_info[:min(len(encoded_target_info), self.max_len)] = encoded_target_info[
                                                                    :min(len(encoded_target_info), self.max_len)]
    ...
    def __input_retrieval(self):
        with open(self.dir + 'retrieved_captions.json', encoding="gb2312") as f:
            retrieval = json.load(f)
        self.retrieval = retrieval


    def __input_feature(self):
        with open(self.dir + 'retrieved_features.json', encoding="gb2312") as f:
            feature = json.load(f)
...
class ours(data.Dataset):  # ours Dataset
    def __init__(self, directory, input_size=(256, 256), random_transform=True,
                 view_pos=['AP', 'PA', 'LATERAL'], max_views=2, sources=['image', 'history'], targets=['label'],
                 max_len=1000, vocab_file='ours_unigram_123.model'):
    ...
    def __getitem__(self, idx):
        #print('Using ours getitem!')
        file_name = self.file_list[idx]
        sources, targets = [], []
        #print(self.file_report)
        tmp_rep = self.captions[self.file_report[file_name]['image'][0] ]
        ...
        if 'image' in self.sources:
            ...
            for i in range(min(self.max_views, len(img_files))):
                img_file = self.dir + 'images/' + img_files[i]
                img = Image.open(img_file).convert('RGB')
                imgs.append(self.transform(img).unsqueeze(0))  # (1,C,W,H)
                vpos.append(1)  # We do not know what view position of the image is, so just let it be 1
    ...
    def __input_label(self):
        with open(self.dir + 'file2label.json', encoding = "gb2312") as f:
            labels = json.load(f)
        self.file_labels = labels

    def __input_caption(self):
        with open(self.dir + 'captions.json', encoding = "gb2312") as f:
            captions = json.load(f)
        self.captions = captions

    def __input_report(self):
        with open(self.dir + 'reports_ori.json', encoding = "gb2312") as f:
            reports = json.load(f)
        self.file_list = [k for k in reports.keys()]
        self.file_report = reports
    ...
    def get_subsets(self, train_size=0.7, val_size=0.1, test_size=0.2, seed=0):
        ...
        train_dataset = ours(self.dir, self.input_size, self.random_transform,
                               self.view_pos, self.max_views, self.sources, self.targets, self.max_len, self.vocab_file)
        train_dataset.file_list = master_file_list[train_indices].tolist()

        # Consider change random_transform to False for validation
        val_dataset = ours(self.dir, self.input_size, False,
                             self.view_pos, self.max_views, self.sources, self.targets, self.max_len, self.vocab_file)
        val_dataset.file_list = master_file_list[val_indices].tolist()

        # Consider change random_transform to False for testing
        test_dataset = ours(self.dir, self.input_size, False,
                              self.view_pos, self.max_views, self.sources, self.targets, self.max_len, self.vocab_file)
        test_dataset.file_list = master_file_list[test_indices].tolist()
