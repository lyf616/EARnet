...
from datasets import MIMIC, NLMCXR , ours
from losses import CELoss
from models_ours import Classifier, TNN
...
DATASET_NAME = 'NLMCXR' # MIMIC / NLMCXR /ours /ours1
MODEL_NAME = 'Transformer' # Transformer / LSTM
MODELS = 'Retrieval_2' #Retrieval/CMN/CG/CR/CGI/KCMN/KCMN_K
...
if __name__ == "__main__":
    ...
    if DATASET_NAME == 'MIMIC':
        ...
        dataset = MIMIC('/workspace/data/files/', INPUT_SIZE, view_pos=['AP','PA','LATERAL'], max_views=MAX_VIEWS, sources=SOURCES, targets=TARGETS)
        ...
        dataset = NLMCXR('./Datasets/NLMCXR/', INPUT_SIZE, view_pos=['AP','PA','LATERAL'], max_views=MAX_VIEWS, sources=SOURCES, targets=TARGETS)
        train_data, val_data, test_data = dataset.get_subsets(seed=123)
        
        VOCAB_SIZE = len(dataset.vocab)
        POSIT_SIZE = dataset.max_len
        COMMENT = 'MaxView{}_NumLabel{}_{}'.format(MAX_VIEWS, NUM_LABELS,MODELS)
    elif DATASET_NAME == 'ours':
        INPUT_SIZE = (256, 256)
        MAX_VIEWS = 2
        NUM_LABELS = 21  # (14 diseases + 100 top noun-phrases)
        NUM_CLASSES = 2
        ...
        INPUT_SIZE = (256, 256)
        MAX_VIEWS = 2
        NUM_LABELS = 5  # (14 diseases + 100 top noun-phrases)
        NUM_CLASSES = 7

        dataset = ours1('./Datasets/ours1/ours/', INPUT_SIZE, view_pos=['AP', 'PA', 'LATERAL'], max_views=MAX_VIEWS,
                         sources=SOURCES, targets=TARGETS)
        train_data, val_data, test_data = dataset.get_subsets(seed=123)
